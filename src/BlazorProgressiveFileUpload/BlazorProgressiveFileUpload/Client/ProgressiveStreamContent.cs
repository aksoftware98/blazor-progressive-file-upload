...
namespace BlazorProgressiveFileUpload.Client
{
    public class ProgressiveStreamContent : StreamContent
    {
        ...
        /// <summary>
        /// Event that we can subscribe to which will be triggered every time after part of the file gets uploaded.
        /// It passes the total amount of uploaded bytes and the percentage as well
        /// </summary>
        public event Action<long, double> OnProgress;

        // Override the SerializeToStreamAsync method which provides us with the stream that we can write our chunks into it
